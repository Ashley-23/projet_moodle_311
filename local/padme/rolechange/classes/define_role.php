 ...
 */
class core_role_define_role extends core_role_capability_table_with_risks {
...
echo "<script> alert('Je suis dvenu après le if') </script>";
    ...
    public function save_allow($type) {
        global $DB;
    ...
    protected function add_permission_cells($capability) {
        ...
        public function ash_role($roleid, array $options) {
            global $DB;
...
echo "<script> alert('Je suis dvenu après le if') </script>";
        $role = $DB->get_record('role', array('id'=>$roleid), '*', MUST_EXIST);
        if ($options['shortname']) {
           $m = $this->role->shortname = $options->shortname;
        }
...
echo "</br>";
 ...
 public function yasmine_role($archetype, array $options) {
        $archetypes = get_role_archetypes();
        if (!isset($archetypes[$archetype])) {
            throw new coding_exception('Unknown archetype: '.$archetype);
        }
        if ($options['shortname']) {
            $this->role->shortname = $options->shortname;
        }
        if ($options['name']) {
            $this->role->name = $options->name;
        }
        if ($options['description']) {
            $this->role->description = $options->description;
        }
        if ($options['archetype']) {
            $this->role->archetype = $archetype;
        }
    ...
    public function kokoe_role() {
        ...
        $rolescount = count($roles);
            // Creating role.
            // $this->role->id = 
            create_role($this->role->name, $this->role->shortname, $this->role->description, $this->role->archetype);
            // $this->roleid = $this->role->id; // Needed to make the parent::save_changes(); call work.
        $this->roleid = $rolescount+1;
