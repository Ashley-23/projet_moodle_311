...
if (isset($_GET['erreur'])) {
	...
	if ($erreur == "echec") {
		...
		echo "Je suis dans la condition 1 :  echec";
	} elseif ($erreur == "lastorfirstnameerror") {
		// (NOM ET PRENOM) EXISTENT DEJA
		$nomprenom_errormessage = "! Le nom et le prénom saisis existent déjà";
		echo "Je suis dans la condition 2 :  nomprenom_error_message";
	} elseif ($erreur == "emailerror") {
		// EMAIL EXISTE DEJA 
		$email_errormessage = "! Cet email existe déjà";
		echo "Je suis dans la condition 3 :  email_errormessage";
	} elseif ($erreur == "usernameerror") {
		// USERNAME EXISTE DEJA 
		$username_errormessage = "! Ce nom d'utilisateur existe déjà";
		echo "Je suis dans la condition 4 :  username_errormessage";
	} elseif ($erreur == "entrepriseerror") {
		// ENTREPRISE EXISTE DEJA
		$entreprise_errormessage = "! Cette entreprise existe déjà";
		echo "Je suis dans la condition 5 :  entreprise_errormessage";
	}
} else {
	echo "";
}

echo "mon erreur est : " . $erreur;
echo "l'id de mon offre est : " . $e$_GET['offre']rreur;
die();
