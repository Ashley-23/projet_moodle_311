...
if (isset($_POST['userfirstname']) || isset($_POST['userlastname']) || isset($_POST['username']) || isset($_POST['useremail']) || isset($_POST['userentreprise1']) || isset($_POST['userentreprise2']) || isset($_POST['userpassword1']) || isset($_POST['userpassword2']) || isset($_POST['offre'])) {
	...
	if ($entreprise1 == $entreprise2 && $mdp1 == $mdp2) {
		...
		if ($cohort_idnumber_result == 1 || $cohort_name_result == 1 || $cohort_idnumber_result == 1 || $cohort_name_result == 1) {
			// 
		...
		$check_user_id = $mysql->query("SELECT * FROM mdl_user WHERE username='$useremail'", MYSQLI_USE_RESULT);

		while ($row = mysqli_fetch_assoc($check_user_id)) {
			$verify_user_id = $row['username'];
		}
		echo "TOUT EST BON? ON PEUT INSERER DANS LES BASES DE DONNEES";




		// VERIFIER LA CREATION DE ROLE SYSTEM
		// VERIFIER L'ATTRIBUTION DE ROLE SYSTEM A UN UTILISATEUR
		// VERIFIER L'ATTRIBUTION DE COHORT A UN UTILISATEUR
