...
if (isset($_POST['userfirstname']) || isset($_POST['userlastname']) || isset($_POST['username']) || isset($_POST['useremail']) || isset($_POST['userentreprise1']) || isset($_POST['userentreprise2']) || isset($_POST['userpassword1']) || isset($_POST['userpassword2']) || isset($_POST['offre'])) {
	...
	if ($entreprise1 == $entreprise2 && $mdp1 == $mdp2) {
		...
		$check_user_firstname = $mysql->query("SELECT * FROM mdl_user WHERE username='$userfirstname'", MYSQLI_USE_RESULT);
		$check_user_lastname = $mysql->query("SELECT * FROM mdl_user WHERE username='$userlastname'", MYSQLI_USE_RESULT);
		$check_user_emailname = $mysql->query("SELECT * FROM mdl_user WHERE username='$useremail'", MYSQLI_USE_RESULT);
		...
		while ($row = mysqli_fetch_assoc($check_user_lastname)last		$verify_user_firstname = $row['firstname'];
		}

		if (empty($verify_user_lastname)) {
			$firstname_result = 0;
		} else {
			if ($verify_user_lastname == $userfirstname) {
				$firstname_result = 1;
			} else {
				$firstname_result = 0;
			}
