...
if (isset($_POST['userfirstname']) || isset($_POST['userlastname']) || isset($_POST['username']) || isset($_POST['useremail']) || isset($_POST['userentreprise1']) || isset($_POST['userentreprise2']) || isset($_POST['userpassword1']) || isset($_POST['userpassword2']) || isset($_POST['offre'])) {
	...
	alert('Je suis rentré')
		...
		while ($row = mysqli_fetch_assoc($sky)) {
			// printf("%s (%s)\n", $row[0], $row[1]);
			// printf("%s (%s)\n", $row['username']);
			echo $row['username'];
			echo $row[7] . "echo 2";
		...
		// $query = "SELECT Name, CountryCode FROM City ORDER BY ID DESC"; LAURENT

		// $result = $mysqli->query($query); LAURENT
		...
		// $donnees = mysql_fetch_array($skya);
		// print_r($skya);
		// echo "<scrpit> alert($skya) </script>";
		// echo "<script> alert('My name is' . $skya) </script>";
		// while ($re = $skya->fetch_array()) {
		// 	print_r($re);
		// }

		// while ($row = mysqli_fetch_array($skya)) {
		// 	$name = $row["firstname"];
		// 	echo "Name: " . $name . "br/>";
		// }
		...
		// if ($resultat = $mysql->use_result($skya)) {
